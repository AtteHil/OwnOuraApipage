...
const getSelectedRange = async () => {
    const activity = await (getDataforDates('activity')); // activity has data evewn if ring has not been on it calculates the acitivty with last data oura has gotten
    if(!activity){
        return -1;
    }
...
const getScores = (sleep, activity, readiness) => {
    let scoreObj = {}
    for (i in activity){
